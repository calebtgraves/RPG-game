...
export default class World {
    ...
    while (emptyTiles.length > 0) {
      ...
      for (const { x, y } of emptyTiles) {
        ...
        if (neighbor) {
          ...
          if (
            neighbor.biome instanceof Water ||
            neighbor.biome instanceof Beach
          ) {
  ...
  private generateMagicalLakes(grid: Tile[][]): void {
    ...
    for (let y = 1; y < this.height - 1; y++) {
      for (let x = 1; x < this.width - 1; x++) {
        ...
        if (tile.biome instanceof Water || tile.biome instanceof Beach)
          continue;
    ...
    for (let y = 0; y < this.height; y++) {
      for (let x = 0; x < this.width; x++) {
        ...
        const allWaterOrEdge = directions.every(([dx, dy]) => {
          ...
          return (
            neighbor.biome instanceof Water ||
            neighbor.biome instanceof DeepWater
          );
        });
    ...
    for (let y = 0; y < this.height; y++) {
      for (let x = 0; x < this.width; x++) {
        ...
        if (waterTiles.length > 500) {
          const largeIslandCount = 1 + Math.floor(Math.random() * 5); // 1-2 large islands
  ...
  private placeIsland(
    ...
    sizeType: "tiny" | "large" = "large",
  ): void {
    // Find tiles that are still water (not already used by another island)
    const availableWater = waterTiles.filter(
      (t) => grid[t.y][t.x]?.biome instanceof Water,
    );
  ...
  private randomBiome(): BiomeConstructor {
    const totalWeight = this.availableBiomes.reduce(
  ...
  private getRandomFilledNeighbor(
    grid: (Tile | null)[][],
    x: number,
    y: number,
  ): Tile | null {
  ...
  findSpawnPoint(): { x: number; y: number } {
    ...
    if (largestLandmass.length > 0) {
      return largestLandmass[
        Math.floor(Math.random() * largestLandmass.length)
      ];
    }
